...
class QuadNode:
    ...
    def __init__(self, bbox, indices_src, indices_tgt,indices_with_neigh, indices_sub_pts, level):
        self.bbox = bbox
...
def points_in_bbox(xyz_src, xyz_tgt, parent, bbox, indices_src, indices_tgt):
    ...
    # mask_w_neigh = (
    #     (xyz_src[:, 0] >= min_b_w_neigh[0]) & (xyz_src[:, 0] < max_b_w_neigh[0]) &
    #     (xyz_src[:, 1] >= min_b_w_neigh[1]) & (xyz_src[:, 1] < max_b_w_neigh[1])
    # )
    mask_w_neigh = (
...
# def points_in_bbox(xyz_src, xyz_tgt, bbox, indices_src, indices_tgt):
#     """Return indices of points inside bbox. xyz: Nx3 array, indices: subset indices."""
#     min_b = bbox.get_min_bound()
#     max_b = bbox.get_max_bound()
#     span = max_b - min_b
...
#     return indices_src[mask], indices_tgt[mask_tgt], np.arange(len(xyz_src))[mask_w_neigh], indices_sub[mask_sub]
...
#     """Return a sub pointcloud from indices."""
#     sub_pc = o3d.geometry.PointCloud()
#     sub_pc.points = o3d.utility.Vector3dVector(
#         np.asarray(pc.points)[indices]
#     )
#     return sub_pc
